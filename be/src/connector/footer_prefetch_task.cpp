...
FooterWarmResult warm_footer(const pipeline::FooterPrefetchItem& item, bool metacache_on) {
    ...
    // Throwaway stats: the footer read/parse path writes counters into the scanner stats.
    HdfsScanStats stats;
    ...
    // FileMetaDataParser reads case_sensitive + split_context from the scanner context and writes
    // its footer counters into ctx.stats. No split (warm always opens the root file); case
    // sensitivity comes from the shared open context.
